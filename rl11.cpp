...
static void execute()
{
    ...
    if (!attached[drv]) {
        Serial.printf("[vpdp1140] RL11 %s drv=%d -> drive not attached\r\n",
                      func_name(func), drv);
    ...
    if (dbg_left > 0) {
        Serial.printf("[vpdp1140] RL11 %s drv=%d  DA=%06o MP=%06o BA=%06o CS=%06o  cur_cyl=%u surf=%u\r\n",
                      func_name(func), drv, RLDA, RLMP, RLBA, RLCS,
                      (unsigned)rl_cur_cyl, (unsigned)rl_cur_surf);
        dbg_left--;
        if (dbg_left == 0)
            Serial.printf("[vpdp1140] RL11 (silencing further command logs)\r\n");
    }
